...
class KreativCommand extends Command
{
    public function __construct(Main $plugin)
    {
        ...
        parent::__construct("gmc", $config->get("prefix") . "Setzt den Spielmodus auf §aKreativ", "/gmc", ["gm1"]);
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): bool
    {
        ...
        if (isset($args[0])) {
            if ($sender->hasPermission("core.command.creativ.other")) {
                ...
                $sender->sendMessage($config->get("error") . "Du hast keine Berechtigung um anderen Spielern den Kreativmodus zu geben!");
                return false;
            }
        }
