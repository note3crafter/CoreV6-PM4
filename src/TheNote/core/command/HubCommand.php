...
class HubCommand extends Command
{
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): bool
    {
        ...
        $name = $config->get("Defaultworld");
        $this->plugin->getServer()->getWorldManager()->loadWorld($name);
        $tp = $this->plugin->getServer()->getWorldManager()->getWorldByName($name);
        if (Server::getInstance()->getWorldManager()->isWorldLoaded($name)) {
            $sender->teleport($tp->getSafeSpawn());
            ...
            $sender->sendMessage($configs->get("error") . "Es ist ein Fehler aufgetreten!");
            return true;
        }
        return true;
    }
