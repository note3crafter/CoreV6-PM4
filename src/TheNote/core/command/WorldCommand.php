...
class WorldCommand extends Command
{
    ...
    public function __construct(Main $plugin)
    {
        $this->plugin = $plugin;
        $api = new BaseAPI();
        parent::__construct("world", $api->getSetting("prefix") . $api->getLang("worldprefix"), "/world");
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): bool
    {
        ...
        $api = new BaseAPI();
        if (empty($args[0])) {
            $sender->sendMessage($api->getSetting("world") . "§6Hilfe");
        ...
        if (isset($args[0])) {
            switch (strtolower($args[0])) {
                ...
                case "tp":
                    ...
                    if (empty($args[1])) {
                        $sender->sendMessage($api->getSetting("info") . "test" . $api->getLang("worldusagetp"));
                        return false;
                    }
                    if (!$this->plugin->getServer()->getWorldManager()->isWorldGenerated($args[1])) {
                        $message = str_replace("{name}", $args[1], $api->getLang("worldnotexist"));
                    ...
                    /*if (!$sender->teleport($level->getSafeSpawn())) {
                        $sender->sendMessage($api->getSetting("error") . "§cDer Teleportvorgang wurde abgebrochen!");
                        return true;
                    }*/
                    $sender->teleport($level->getSpawnLocation());
                    $message = str_replace("{world}", $args[1], $api->getLang("worldtpsucces"));
                    $sender->sendMessage($api->getSetting("world") . $message);
                    break;
                case "delete":
                    if (empty($args[1])) {
                        $sender->sendMessage($api->getSetting("info") . $api->getLang("worldusagedelete"));
                        return false;
                    }
                    ...
                    if (!$this->plugin->getServer()->getWorldManager()->isWorldGenerated($args[0]) and !file_exists($this->plugin->getServer()->getDataPath() . "worlds/" . $args[1])) {
                        if ($this->plugin->getServer()->getWorldManager()->getDefaultWorld() === $this->plugin->getServer()->getWorldManager()->getWorldByName($args[1])) {
                            $sender->sendMessage($api->getSetting("error") . $api->getLang("worlddefaultcantdelete"));
                        } else {
                            $message = str_replace("{name}", $args[1], $api->getLang("worldnotexist"));
                            $sender->sendMessage($api->getSetting("error") . $message);
                            return false;
                        ...
                        $message = str_replace("{world}", $args[1], $api->getLang("worlddeletet"));
                        $sender->sendMessage($api->getSetting("world") . $message);
                        $message1 = str_replace("{files}", $files, $api->getLang("worldfilesdeletet"));
                        $sender->sendMessage($api->getSetting("world") . $message1);
                    }
                    break;
                case "list":
                    ...
                    $message = str_replace("{worlds}", (string)count($levels), $api->getLang("worldsloaded"));
                    $sender->sendMessage($api->getSetting("world") . $message);
                    foreach ($levels as $level => [$loaded, $players]) {
                        $loaded = $loaded ? $api->getLang("worldload") : $api->getLang("worldunload");
                        $sender->sendMessage("§7{$level} > {$loaded} " . $api->getLang("worldplayer") . " {$players}");
                    }
                    break;
                case "create":
                    if (empty($args[1])) {
                        $sender->sendMessage($api->getSetting("info") . $api->getLang("worldusagecreate"));
                        return false;

                    }
                    if ($this->isLevelGenerated($args[1])) {
                        $message = str_replace("{name}", $args[1], $api->getLang("worldalreadyexist"));
                        $sender->sendMessage($api->getSetting("error") . $message);
                        return false;
                    }
                    $seed = 0;
                    ...
                    $message = str_replace("{name}", $args[1], $api->getLang("worldsucces"));
                    $message1 = str_replace("{seed}", $seed, $message);
                    $message2 = str_replace("{generator}", $generatorName, $message1);
                    $sender->sendMessage($api->getSetting("world") . $message2);
                    break;
