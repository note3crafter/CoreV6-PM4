...
class sauer extends Command {
    ...
    public function __construct(Main $plugin)
    {
        $this->plugin = $plugin;
        $api = new BaseAPI();
        parent::__construct("sauer", $api->getSetting("prefix") . $api->getLang("angryprefix"), "/sauer");
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args)
    {
        ...
        if (!$sender instanceof Player) {
            return $this->plugin->getServer()->broadcastMessage($api->getLang("angryserver"));
        }
        ...
        $message = str_replace("{player}", $nickname, $api->getLang("angrysucces"));
