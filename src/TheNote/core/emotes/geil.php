...
class geil extends Command {
    ...
    public function __construct(Main $plugin)
    {
        $this->plugin = $plugin;
        $api = new BaseAPI();
        parent::__construct("geil", $api->getSetting("prefix") . $api->getLang("geilprefix"), "/geil");
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args)
    {
        ...
        if (!$sender instanceof Player) {
            return $this->plugin->getServer()->broadcastMessage($api->getLang("geilsucces"));
        }
        ...
        $message = str_replace("{player}", $nickname, $api->getLang("geilsucces"));
